'use client';

import { useState } from 'react';
import Image from "next/image";

interface PhiPhiIslandSectionProps {
  openImageModal: (src: string, alt: string) => void;
}

const PhiPhiIslandSection = ({ openImageModal }: PhiPhiIslandSectionProps) => {
  const [loadedImages, setLoadedImages] = useState<Set<string>>(new Set());

  // Handle image loading
  const handleImageLoad = (imageUrl: string) => {
    setLoadedImages(prev => new Set([...prev, imageUrl]));
  };

  const galleryImages = [
    {
      src: "/image/day-trips/destinations/phi-phi1.jpg",
      alt: "Maya Bay and limestone cliffs",
    },
    {
      src: "/image/day-trips/destinations/phi-phi2.jpg",
      alt: "Tropical paradise beaches",
    },
    {
      src: "/image/day-trips/destinations/phi-phi3.jpg",
      alt: "Crystal clear waters and pristine beaches",
    },
    {
      src: "/image/day-trips/destinations/phi-phi4.jpg",
      alt: "Phi Phi Island scenic views",
    },
  ];

  return (
    <section className="py-4 bg-white relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-100 rounded-full translate-x-48 -translate-y-48"></div>
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-gray-100 rounded-full -translate-x-48 translate-y-48"></div>
      </div>

      <div className="max-w-6xl mx-auto px-4 relative z-10">
        {/* Header Section */}
        <div className="text-center mb-16">
          {/* Section Badge */}
          <div className="inline-block mb-6">
            <span className="px-4 py-1 border border-gray-300 text-gray-600 text-xs font-medium uppercase tracking-widest">
              Featured Destination
            </span>
          </div>

          {/* Title */}
          <h2 className="text-4xl lg:text-5xl font-light text-gray-900 mb-6 tracking-tight leading-tight">
            Phi Phi Island Day Trip
          </h2>

          {/* Description */}
          <p className="text-lg text-gray-600 font-light leading-relaxed max-w-2xl mx-auto">
            Experience the breathtaking beauty of Maya Bay and crystal clear waters
            in Thailand&apos;s most famous island paradise.
          </p>
        </div>

        {/* Main Content Grid */}
        <div className="grid lg:grid-cols-2 gap-16 mb-16">
          {/* Left Column - Image Gallery */}
          <div>
            <h3 className="text-2xl font-light text-gray-900 mb-4 tracking-wide">
              Gallery
            </h3>
            <div className="grid grid-cols-2 gap-1 mb-12">
              {galleryImages.map((image, index) => {
                const isLoaded = loadedImages.has(image.src);

                return (
                  <div
                    key={index}
                    className={`relative aspect-square overflow-hidden group cursor-pointer ${index === 0 ? 'col-span-2 aspect-[2/1]' : ''
                      }`}
                    onClick={() => openImageModal(image.src, image.alt)}
                  >
                    {/* Loading placeholder with shimmer */}
                    <div
                      className={`absolute inset-0 bg-gray-200 transition-opacity duration-700 ${isLoaded ? 'opacity-0' : 'opacity-100'
                        }`}
                    >
                      <div className="absolute inset-0 bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 animate-pulse"></div>
                      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent animate-shimmer"></div>
                    </div>

                    {/* Main image with fade-in animation */}
                    <Image
                      src={image.src}
                      alt={image.alt}
                      fill
                      className={`object-cover group-hover:scale-110 transition-all duration-700 ${isLoaded
                        ? 'opacity-100 scale-100'
                        : 'opacity-0 scale-105'
                        }`}
                      onLoad={() => handleImageLoad(image.src)}
                      placeholder="blur"
                      blurDataURL="data:image/jpeg;base64,"
                    />

                    {/* Hover overlay with alt text */}
                    <div className={`absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center ${isLoaded ? '' : 'pointer-events-none'
                      }`}>
                      <span className="text-white text-sm font-light text-center px-4 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                        {image.alt}
                      </span>
                    </div>
                  </div>
                );
              })}
            </div>

            {/* About Section */}
            <div>
              <h3 className="text-2xl font-light text-gray-900 mb-4 tracking-wide">
                About Phi Phi Islands
              </h3>
              <div className="space-y-2">
                <p className="text-gray-700 font-light leading-relaxed">
                  Koh Phi Phi islands are located 42 km from Phuket in the Andaman Sea.
                  Part of Nopharat Thara Beach – Phi Phi Islands National Park, featuring
                  stunning limestone cliffs and pristine beaches.
                </p>
                <p className="text-gray-700 font-light leading-relaxed">
                  Maya Bay was featured in the Hollywood movie &quot;The Beach&quot; and epitomizes
                  the stunning beauty of these islands with pellucid aquamarine water
                  surrounded by towering limestone cliffs.
                </p>
              </div>
            </div>
          </div>

          {/* Right Column - Tour Information */}
          <div>
            {/* Tour Features */}
            <div className="mb-12">
              <h3 className="text-2xl font-light text-gray-900 mb-4 tracking-wide">
                Tour Features
              </h3>
              <div className="space-y-2">
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Visit Maya Bay (The Beach movie location)</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Snorkeling in crystal clear waters</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Explore Phi Phi Don and Phi Phi Le</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Lunch at beachside restaurant</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Swimming and beach relaxation</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Professional snorkeling equipment provided</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Safe transportation with child seats</span>
                </div>
              </div>
            </div>

            {/* Key Tips */}
            <div className="mb-12">
              <h3 className="text-2xl font-light text-gray-900 mb-4 tracking-wide">
                Key Tips
              </h3>
              <div className="space-y-2">
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Best time to visit: November to April</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Perfect for snorkeling and diving</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Vibrant nightlife at Tonsai Beach</span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1 h-1 bg-gray-400 rounded-full mt-3 flex-shrink-0"></div>
                  <span className="text-gray-700 font-light">Exotic marine life including leopard sharks</span>
                </div>
              </div>
            </div>

            {/* Family Friendly Info */}
            <div className="bg-gray-50 border border-gray-200 p-8">
              <h4 className="text-lg font-light text-gray-900 mb-4 tracking-wide">
                Family Friendly
              </h4>
              <p className="text-gray-700 font-light leading-relaxed">
                Suitable for all ages with shallow snorkeling areas and calm beaches
                perfect for children. Professional guides ensure safety for the whole family.
              </p>
            </div>
          </div>
        </div>

        {/* CTA Section */}
        <div className="text-center">
          <button
            onClick={() => window.location.href = '/contact'}
            className="group relative overflow-hidden bg-gray-900 text-white font-light text-sm uppercase tracking-wider px-12 py-4 transition-all duration-300 hover:bg-gray-800"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            <span className="relative z-10">Book This Tour</span>
          </button>
        </div>
      </div>
    </section>
  );
};

export default PhiPhiIslandSection;
